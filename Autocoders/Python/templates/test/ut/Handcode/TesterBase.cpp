...
namespace ExampleComponents {
    ...
    )
  {
    ...
    // Initialize output port anotherInput

    for (
        NATIVE_INT_TYPE _port = 0;
        _port < this->getNum_to_anotherInput();
        ++_port
    ) {
      this->m_to_anotherInput[_port].init();
      ...
      this->m_to_anotherInput[_port].setObjName(_portName);
#endif
  ...
  NATIVE_INT_TYPE ExampleTesterBase ::
    getNum_to_anotherInput(void) const
  {
    return (NATIVE_INT_TYPE) FW_NUM_ARRAY_ELEMENTS(this->m_to_anotherInput);
  }
  ...
  SomeOtherNamespace::AnotherEnum ExampleTesterBase ::
    invoke_to_anotherInput(
        const NATIVE_INT_TYPE portNum,
        I32 arg1,
        F64 arg2,
        SomeOtherNamespace::SomeEnum arg3
    )
  {
    FW_ASSERT(portNum < this->getNum_to_anotherInput(),static_cast<AssertArg>(portNum));
    FW_ASSERT(portNum < this->getNum_to_anotherInput(),static_cast<AssertArg>(portNum));
    return this->m_to_anotherInput[portNum].invoke(
        arg1, arg2, arg3
    );
  }
  ...
  bool ExampleTesterBase ::
    isConnected_to_anotherInput(const NATIVE_INT_TYPE portNum)
  {
    FW_ASSERT(portNum < this->getNum_to_anotherInput(), static_cast<AssertArg>(portNum));
    return this->m_to_anotherInput[portNum].isConnected();
  }
    ...
    )
  {
    ...
    switch (id - idBase) {
      ...
      case ExampleComponentBase::EVENTID_SOMEEVENT:
      {
        ...
        this->logIn_ACTIVITY_HI_SomeEvent(arg1, arg2, arg3, arg4, arg5);
    ...
    )
  {
    ...
    _prmOpcode =  ExampleComponentBase::OPCODE_SOMEPARAM_SET + idBase;
...

  {
    ...
    FwOpcodeType _prmOpcode;
    const U32 idBase = this->getIdBase();
    _prmOpcode = ExampleComponentBase::OPCODE_SOMEPARAM_SAVE + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
    ...
    )
  {
    ...
    const U32 idBase = this->getIdBase();
    FwOpcodeType _prmOpcode;
    _prmOpcode =  ExampleComponentBase::OPCODE_ANOTHERPARAM_SET + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
...

  {
    ...
    FwOpcodeType _prmOpcode;
    const U32 idBase = this->getIdBase();
    _prmOpcode = ExampleComponentBase::OPCODE_ANOTHERPARAM_SAVE + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
    ...
    )
  {
    ...
    const U32 idBase = this->getIdBase();
    FwOpcodeType _prmOpcode;
    _prmOpcode =  ExampleComponentBase::OPCODE_STRINGPARAM_SET + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
...

  {
    ...
    FwOpcodeType _prmOpcode;
    const U32 idBase = this->getIdBase();
    _prmOpcode = ExampleComponentBase::OPCODE_STRINGPARAM_SAVE + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
    ...
    )
  {
    ...
    const U32 idBase = this->getIdBase();
    FwOpcodeType _prmOpcode;
    _prmOpcode =  ExampleComponentBase::OPCODE_ENUMPARAM_SET + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
...

  {
    ...
    FwOpcodeType _prmOpcode;
    const U32 idBase = this->getIdBase();
    _prmOpcode = ExampleComponentBase::OPCODE_ENUMPARAM_SAVE + idBase;
    if (not this->m_to_CmdDisp[0].isConnected()) {
      printf("Test Command Output port not connected!\n");
    }
    else {
